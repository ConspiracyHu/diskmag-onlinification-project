	...
	formats.forEach(function(archive_format) {
		...
		switch (archive_format) {
			...
			case 'zip':
				...
				break;
			case 'crdata00':
				loadScript(path + 'libcrdata00.js', checkForLoadDone);
				_loaded_archive_formats.push(archive_format);
				break;
			case 'tar':
...
function archiveOpenArrayBuffer(file_name, password, array_buffer) {
	...
	if (isRarFile(array_buffer)) {
		archive_type = 'rar';
	} else if(isCRDATA00File(array_buffer)) {
		archive_type = 'crdata00';
	...
	try {
		switch (archive_type) {
			case 'rar':
				...
				break;
			case 'crdata00':
				handle = _crdata00Open(file_name, password, array_buffer);
				entries = _crdata00GetEntries(handle);
				break;
			case 'zip':
...
function _crdata00Open(file_name, password, array_buffer) {
	return {
		file_name: file_name,
		array_buffer: array_buffer,
	};
...
function _crdata00GetEntries(crdata00_handle) {
	let crdata00_entries = crdata00GetEntries(crdata00_handle.file_name, crdata00_handle.array_buffer);
	
	// Get all the entries
	let entries = [];
	crdata00_entries.forEach(function(entry) {
		let name = entry.name;
		...
		entries.push({
			...
			readData: function(cb) {
				setTimeout(function() {
					let data = crdata00GetEntryData(entry, crdata00_handle.array_buffer);
					cb(data.buffer, null);
				}, 0);
			}
		});
...
function isCRDATA00File(array_buffer) {
	// Just return false if the file is smaller than the header
	if (array_buffer.byteLength < 8) {
		return false;
	}
	...
	// Return true if the header matches the ZIP header
	let crdata00_header = saneJoin(['C', 'R', 'D', 'A', 'T', 'A', '0', '0'], ', ');
	let header = saneJoin(saneMap(new Uint8Array(array_buffer).slice(0, 8), String.fromCharCode), ', ');
	return (header === crdata00_header);
}
